...
const Preloader: React.FC = () => {
    ...
    return (
        <motion.div variants={slideUp} initial="initial" exit="exit" className=" h-screen w-screen flex items-center justify-center fixed z-1000 bg-white">
            {dimension.width > 0 && 
            <>
                <motion.p variants={opacity} initial="initial" animate="enter" className={`flex text-black text-4xl items-center absolute z-10 ${poppins.className}`}><span className=' block w-3 h-3 bg-black mr-3 rounded-full'></span>{words[index]}</motion.p>
